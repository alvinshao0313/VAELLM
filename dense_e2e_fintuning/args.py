...
_DEFAULT_RUN_ROOT = ".result/dense_e2e_fintuning"
...
@dataclass
class DenseE2EFinetuneArguments:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Dense replay e2e finetuning from compressed checkpoints.")
...
def _validate_dataset_inputs(parser: argparse.ArgumentParser, args: DenseE2EFinetuneArguments) -> None:
...
def _validate_numeric_inputs(parser: argparse.ArgumentParser, args: DenseE2EFinetuneArguments) -> None:
...
def _validate_variant_inputs(
    ...
    args: DenseE2EFinetuneArguments,
...
def validate_args(
    ...
    explicit_cli_flags = set(getattr(args, "explicit_cli_flags", []) or [])
    if "--student_model_path" in explicit_cli_flags:
        parser.error("dense 训练轨不接受 --student_model_path，请使用 --student_checkpoint_dir。")
    ...
    decode_device = str(getattr(args, "decode_device", "")).strip().lower()
...
def parse_args(
...
) -> Tuple[DenseE2EFinetuneArguments, HFArguments, TrainingArguments]:
    raw_argv = list(sys.argv[1:] if argv is None else argv)
    parser = build_parser()
    raw_ns, remaining = parser.parse_known_args(raw_argv)
    dense_args = DenseE2EFinetuneArguments(**vars(raw_ns))
    dense_args.explicit_cli_flags = _collect_explicit_cli_flags(raw_argv)
    explicit_flags = set(dense_args.explicit_cli_flags or [])
    if "--student_model_path" in explicit_flags:
        parser.error("dense 训练轨不接受 --student_model_path，请使用 --student_checkpoint_dir。")
    legacy_dense_flags = sorted(
        flag
        for flag in explicit_flags
        if flag.startswith("--vae_lora_") or flag.startswith("--vae_adalora_")
    )
    if legacy_dense_flags:
        parser.error(
            "dense 训练轨不再接受旧的 vae 前缀 LoRA 参数，请改用无前缀名字："
            + ",".join(legacy_dense_flags)
        )
    ...
    validate_args(parser, dense_args, training_args)
    return dense_args, hf_args, training_args
