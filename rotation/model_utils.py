...
SAME_TYPE_MODELS = [LLAMA_MODEL, MISTRAL_MODEL, QWEN2_MODEL, QWEN3_MODEL]
...
def get_mistral(model_name, hf_token):
    ...
    torch.nn.init.uniform_ = skip
    torch.nn.init.normal_ = skip
    model = transformers.MistralForCausalLM.from_pretrained(model_name, torch_dtype='auto',
                                                            use_auth_token=hf_token,
                                                            low_cpu_mem_usage=True)
    model.seqlen = 2048
    logging.info('---> Loading {} Model with seq_len: {}'.format(model_name, model.seqlen))
...
def get_embeddings(model, model_type) -> list[torch.nn.Module]:
    if model_type in SAME_TYPE_MODELS:
...
def get_mlp_bottleneck_size(model):
    ...
    if model_type in [LLAMA_MODEL, MISTRAL_MODEL, QWEN2_MODEL, QWEN3_MODEL]:
...
def replace_modules(
    ...
    for name, module in root.named_children():
        if expect_list and name in expect_list:
            continue
        new_module = None
...
class QwenRMSN(torch.nn.Module):
