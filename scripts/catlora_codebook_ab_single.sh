...
# 单 trial 入口：GPU 由第一个参数指定，类别/codebook/output_dir 等通过 "$@" 覆盖。
# 用法: bash scripts/catlora_codebook_ab_single.sh <GPU_ID> [extra args...]

if [ $# -lt 1 ]; then
  echo "Usage: bash $0 <CUDA_VISIBLE_DEVICES> [extra args...]"
  echo "Example: bash $0 4 --target_categories q_proj --codebook_bits default=32 --codebook_dim default=32 --residual_stages default=2 --output_dir .result/catlora_codebook_ab/q_proj/b32d32s2"
  exit 1
fi
