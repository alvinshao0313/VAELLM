# 联合优化代码，已关闭。
# 下面保留旧 joint decoder 联合微调实现，仅用于历史审阅；cat_train 不再 import 或调用本文件。
# import time
...
#         raise ValueError("排序代码已关闭；joint decoder 只支持 intra_part_sort_mode=none。")
...
#
#
# def build_identity_joint_restore_plan(
#     *,
#     common_shape: Tuple[int, int, int],
#     num_stages: int,
# ) -> JointRestorePlan:
...
#     num_blocks, num_models, block_dim = [int(v) for v in common_shape]
#     total_numel = int(num_blocks) * int(num_models) * int(block_dim)
#     src_idx_global = torch.arange(total_numel, dtype=torch.long).view(num_blocks, num_models, block_dim)
#     src_idx_flat = src_idx_global.reshape(-1).contiguous()
#     return JointRestorePlan(
#         common_shape=(num_blocks, num_models, block_dim),
#         stage_plans=[
#             JointStageRestorePlan(
#                 stage_src_idx_global=src_idx_global.contiguous(),
#                 stage_src_idx_flat=src_idx_flat,
#             )
#             for _stage_idx in range(int(num_stages))
#         ],
#     )
#
#
...
#     # 排序代码，已关闭：joint decoder 只处理 common layout，无需构造排序 restore plan。
#     joint_restore_plan = build_identity_joint_restore_plan(
#         common_shape=tuple(int(v) for v in target_common.shape),
#         num_stages=len(all_stage_bits),
#     )
...
#     stage_src_idx_flat_on_device: List[Optional[torch.Tensor]] = []
...
#         stage_src_idx_flat_on_device = [None for _stage_plan in joint_restore_plan.stage_plans]
#
