...
_SELECTOR_DEFAULT = "default"
_SELECTOR_CATEGORY = "cat"
_SELECTOR_AFTER = "after"
_KNOWN_SELECTORS = (_SELECTOR_DEFAULT, _SELECTOR_CATEGORY, _SELECTOR_AFTER)
...
class OverrideKey:
    ...
    def as_token(self) -> str:
        if self.selector == _SELECTOR_DEFAULT:
            return _SELECTOR_DEFAULT
...
def _parse_selector(selector_token: str, spec: OverrideSpec[T]) -> OverrideKey:
    ...
    if raw == _SELECTOR_DEFAULT:
        selector = OverrideKey(selector=_SELECTOR_DEFAULT, value=None)
    elif raw.startswith(f"{_SELECTOR_CATEGORY}:"):
        category = raw.split(":", 1)[1].strip()
        if not category:
            raise argparse.ArgumentTypeError(
                f"Invalid {spec.arg_name} selector '{raw}'. Category name cannot be empty."
            )
        selector = OverrideKey(selector=_SELECTOR_CATEGORY, value=category)
    elif raw.startswith(f"{_SELECTOR_AFTER}:"):
        category = raw.split(":", 1)[1].strip()
        if not category:
            raise argparse.ArgumentTypeError(
                f"Invalid {spec.arg_name} selector '{raw}'. after-category name cannot be empty."
            )
        selector = OverrideKey(selector=_SELECTOR_AFTER, value=category)
    else:
        raise argparse.ArgumentTypeError(
...
def parse_override_table(raw: str, spec: OverrideSpec[T]) -> OverrideTable[T]:
    ...
    for item in items:
        ...
        if selector.selector == _SELECTOR_DEFAULT:
            table.has_default = True
            table.default = parsed_value
        elif selector.selector == _SELECTOR_CATEGORY:
            table.by_category[str(selector.value)] = parsed_value
        elif selector.selector == _SELECTOR_AFTER:
